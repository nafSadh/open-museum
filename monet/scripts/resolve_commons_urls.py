...
INPUT_PATH = "monet/wiki_works_raw.json"
OUTPUT_PATH = "monet/catalog.json"
...
def fetch_image_info(filenames: list[str]) -> dict:
    ...
    for page_id, page_data in pages.items():
        ...
        # Sometime wikipedia gives filename with urlencoded characters
        fname_unquoted = urllib.parse.unquote(fname)
...
def main():
    ...
    for i, work in enumerate(works):
        ...
        if fn:
            ...
            fn_norm = fn.replace(" ", "_")
            if fn_norm not in fname_to_indices:
                fname_to_indices[fn_norm] = []
                filenames_to_resolve.append(fn_norm)
            fname_to_indices[fn_norm].append(i)
    ...
    print(f"Resolved {len(resolved)//2} filenames (approx)") # divided by two because we store quoted and unquoted

    # Merge resolved data back into works
    resolved_count = 0
    for fn_norm, indices in fname_to_indices.items():
        # try unquoted as well
        unq = urllib.parse.unquote(fn_norm)
        info = resolved.get(fn_norm) or resolved.get(unq)
        if info:
            for i in indices:
