#!/usr/bin/env python3
"""
Scrape the Wikipedia "List of paintings by Johannes Vermeer" page.
Two tables: confirmed paintings (34) and disputed paintings (1).
Columns: Image, Title, Year, Size, Location

Outputs: vermeer/wiki_works_raw.json
"""
...
PAGE_TITLE = "List of paintings by Johannes Vermeer"
OUTPUT_PATH = "vermeer/wiki_works_raw.json"
...
def process_table(table: dict, disputed: bool = False) -> list:
    """Columns: Image, Title, Year, Size, Location"""
    works = []
    for row in table["rows"]:
        ...
        # Col 3: Size (often includes medium, e.g. "Oil on canvas, 160 x 142 cm")
        if len(row) > 3:
            size_text = row[3]["text"].strip()
            # Split medium from dimensions
            if "," in size_text:
                parts = size_text.split(",", 1)
                work["technique"] = parts[0].strip()
                work["dimensions"] = parts[1].strip()
            else:
                work["dimensions"] = size_text

        # Col 4: Location
        if len(row) > 4:
            work["current_location"] = row[4]["text"].strip()
...
def main():
    ...
    all_works = []
    for i, table in enumerate(extractor.tables):
        disputed = i > 0
        works = process_table(table, disputed=disputed)
        label = "disputed" if disputed else "confirmed"
        print(f"  Table {i+1}: {len(works)} {label} works")
