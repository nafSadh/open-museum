...
Outputs: xu-beihong/catalog.json
"""
...
INPUT_PATH = "xu-beihong/wiki_works_raw.json"
OUTPUT_PATH = "xu-beihong/catalog.json"
...
def main():
    ...
    # Build a lookup that normalizes spaces/underscores for matching
    resolved_normalized = {}
    for rk, rv in resolved.items():
        resolved_normalized[rk] = rv
        resolved_normalized[rk.replace("_", " ")] = rv
        resolved_normalized[rk.replace(" ", "_")] = rv

    # Merge resolved data back into works
    resolved_count = 0
    for fn, indices in fname_to_indices.items():
        info = resolved_normalized.get(fn) or resolved_normalized.get(fn.replace(" ", "_")) or resolved_normalized.get(fn.replace("_", " "))
        if info:
            for i in indices:
